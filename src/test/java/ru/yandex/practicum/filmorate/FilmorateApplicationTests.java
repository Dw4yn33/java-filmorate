...
class FilmorateApplicationTests {
	...
	public void deleteFilm() {
		firstFilm = filmStorage.create(firstFilm);
		secondFilm = filmStorage.create(secondFilm);
		filmStorage.deleteFilmById(firstFilm.getId());
		List<Film> listFilms = filmStorage.getFilms();
	...
	public void testGetCommonFriends() {
		...
		assertThat(userService.getSharedFriends(firstUser.getId(), secondUser.getId())).hasSize(1);
		assertThat(userService.getSharedFriends(firstUser.getId(), secondUser.getId()))
				.contains(thirdUser);
	}
}
